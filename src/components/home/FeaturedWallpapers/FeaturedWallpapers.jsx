...
const FeaturedWallpapers = () => {
  const { data } = useGetFeaturedWallpapersQuery();
          ...
          <div className="md:grid grid-cols-3 gap-x-[55px] hidden md:inline-block">
            {data?.data?.map((item, index) => (
              <div
                ...
                className="min-h-[279px] max-h-[279px] max-w-[494px] w-full rounded-[15px] overflow-hidden relative"
              >
                <LazyWallpaper
                  src={item?.wallpaper}
                  alt={item?.wallpaper}
          ...
          <div className="grid grid-cols-2 gap-x-[6px] gap-y-[10px] md:hidden">
            {data?.data?.map((item, index) => (
              ...
              >
                <img
                  onClick={() => navigate(`/w/${item?.slug}`)}
                  src={viewResizeImg(item?.wallpaper)}
                  alt="img"
                  loading="lazy"
                  className="w-full h-full object-cover hover:scale-110 duration-300 cursor-pointer"
                />
